...
async def git_on_message(bot, update):
    if not len(update.command) == 2:
        message = "Please write release repository into it, i.e <code>/git user/repo</code>"
...
async def rm_repo(bot, update):
    if not len(update.command) == 1:
        message = "Please write release repository into it, i.e <code>/gitdel (name)</code>"
        await bot.send_message(
                chat_id=update.chat.id,
                text=message
            )
